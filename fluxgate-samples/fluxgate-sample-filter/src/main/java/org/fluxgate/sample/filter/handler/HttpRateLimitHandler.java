 ...
 * <p>
 * This handler calls an external FluxGate API server to check rate limits.
 * Suitable for:
 * <ul>
 *   <li>Distributed deployments without direct Redis access</li>
 *   <li>Microservices that need centralized rate limiting</li>
 *   <li>Applications behind a rate limiting gateway</li>
 * </ul>
 * <p>
 * Configuration:
 * <pre>
...
public class HttpRateLimitHandler implements FluxgateRateLimitHandler {
    ...
    public HttpRateLimitHandler(
            @Value("${fluxgate.api.url:http://localhost:8080}") String apiUrl) {
        this.apiUrl = apiUrl;
    ...
    public RateLimitResponse tryConsume(RequestContext context, String ruleSetId) {
        try {
            RateLimitApiResponse response = restClient.post()
