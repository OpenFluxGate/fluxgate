...
public class StandaloneRateLimitHandler implements FluxgateRateLimitHandler {
    ...
    public RateLimitResponse tryConsume(RequestContext context, String ruleSetId) {
        log.debug("Processing rate limit for ruleSetId: {}, clientIp: {}",
                ruleSetId, context.getClientIp());
        ...
        if (result.isAllowed()) {
            log.debug("Request allowed for ruleSet: {}, remaining: {}",
                    ruleSetId, result.getRemainingTokens());
            return RateLimitResponse.allowed(
                    result.getRemainingTokens(),
                    0  // reset time not directly available from RateLimitResult
            );
        } else {
            long retryAfterMillis = result.getNanosToWaitForRefill() / 1_000_000;
            log.info("Request rejected for ruleSet: {}, retry after: {} ms",
                    ruleSetId, retryAfterMillis);
