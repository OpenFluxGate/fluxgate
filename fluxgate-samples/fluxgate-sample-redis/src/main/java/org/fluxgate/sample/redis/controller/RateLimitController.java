 ...
 * <p>
 * This controller provides the HTTP API endpoint that client applications
 * (like fluxgate-sample-filter) can call to check rate limits.
 */
@RestController
...
public class RateLimitController {
    ...
    @PostMapping("/check")
    public ResponseEntity<RateLimitCheckResponse> checkRateLimit(@RequestBody RateLimitCheckRequest request) {
        log.debug("Rate limit check request: ruleSetId={}, clientIp={}",
                request.ruleSetId, request.clientIp);
        ...
        if (result.isAllowed()) {
            response = new RateLimitCheckResponse(
                    true,
    ...
    private RateLimitRuleSet buildRuleSet(RuleSetData data, RateLimitCheckRequest request) {
        ...
        KeyResolver keyResolver = context -> {
            String keyValue = switch (data.getKeyStrategyId()) {
