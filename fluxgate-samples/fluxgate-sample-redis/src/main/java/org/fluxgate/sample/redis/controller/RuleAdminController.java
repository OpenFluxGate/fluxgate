...
public class RuleAdminController {
    ...
    public ResponseEntity<Map<String, Object>> createRuleSet(
        ...
        log.info("Creating RuleSet: {} with capacity={}, windowSeconds={}",
    ...
    public ResponseEntity<Map<String, Object>> createSampleRuleSet() {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(Map.of(
                ...
                "hint", "Try: for i in {1..10}; do curl -s http://localhost:8082/api/test?ruleSetId=test-limits; echo; sleep 0.5; done"
        ));
    }

    @Operation(
            summary = "List all RuleSets",
            description = "Returns all registered RuleSet IDs"
    )
    ...
    @Operation(
            ...
            description = "Removes a RuleSet by ID"
    )
    ...
    public ResponseEntity<Void> deleteRuleSet(
            @Parameter(description = "RuleSet ID", required = true)
