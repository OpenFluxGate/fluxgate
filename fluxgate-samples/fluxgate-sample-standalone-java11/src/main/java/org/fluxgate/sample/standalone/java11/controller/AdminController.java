...
public class AdminController {
  ...
  public ResponseEntity<Map<String, Object>> createMultiFilterRules() {
    ...
    Map<String, Object> rule1Info = new HashMap<>();
    rule1Info.put("ruleId", rule1.getId());
    rule1Info.put("limit", "10 requests per minute");
    rules.put("rule1", rule1Info);
    Map<String, Object> rule2Info = new HashMap<>();
    rule2Info.put("ruleId", rule2.getId());
    rule2Info.put("limit", "20 requests per minute");
    rules.put("rule2", rule2Info);
                      ...
                      rule.getBands().stream()
                          .map(
                              band -> {
                                ...
                                bandMap.put("label", band.getLabel());
                                bandMap.put("capacity", band.getCapacity());
                                bandMap.put("windowSeconds", band.getWindow().toSeconds());
                                return bandMap;
                              })
