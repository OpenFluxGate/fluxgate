 ...
 *  - A "PER_IP, 100 req/min" rule set stored in MongoDB
 *  - Requests #1 ~ #100 are allowed
 *  - Request #101 is rejected
 *  - Metrics are recorded into MongoDB
 *
 * This test uses a simple in-memory RateLimiter implementation
 * so that only the Mongo adapter + metrics adapter are exercised.
 */
class MongoRateLimitIntegrationTest {
    ...
    private static final String MONGO_URI =
            System.getProperty("fluxgate.mongo.uri",
                    System.getenv().getOrDefault("FLUXGATE_MONGO_URI", DEFAULT_MONGO_URI));

    private static final String DB_NAME =
            System.getProperty("fluxgate.mongo.db",
                    System.getenv().getOrDefault("FLUXGATE_MONGO_DB", DEFAULT_DB_NAME));
    ...
    void mongoRateLimitTest() {
        ...
        for (int i = 1; i <= 100; i++) {
            ...
            System.out.println("  - Request #" + i + " allowed=" + result.isAllowed()
    ...
    void mongoRateLimitWindowResetTest() throws InterruptedException {
        ...
        for (int i = 1; i <= 100; i++) {
            RateLimitResult result = limiter.tryConsume(ctx, ruleSet);
            System.out.println("  - [Window#1] Request #" + i + " allowed=" + result.isAllowed()
                    + ", remaining=" + result.getRemainingTokens());
        ...
        System.out.println("  - [Window#2] Request #1 allowed=" + resultAfterWindow.isAllowed()
                + ", remaining=" + resultAfterWindow.getRemainingTokens());
        assertTrue(resultAfterWindow.isAllowed(),
    ...
    /**
     * Simple in-memory RateLimiter used only in this test.
     * It assumes a single RateLimitRule in the provided RateLimitRuleSet.
     * This implementation uses System.nanoTime() and respects the first band window.
     */
    private static class InMemoryTestRateLimiter implements RateLimiter {
        ...
        public RateLimitResult tryConsume(RequestContext context,
            ...
            if (windowNanos < 0L) {
                ...
                System.out.println("  - [Window] initialized: window=" + window +
                        ", windowNanos=" + windowNanos +
                        ", startNanos=" + windowStartNanos);
            }
            ...
            if (now - windowStartNanos >= windowNanos) {
                System.out.println("  - [Window] expired, resetting (now=" + now +
                        ", startNanos=" + windowStartNanos + ")");
