 ...
 * <p>
 * This test demonstrates the full FluxGate architecture:
 * 1. Rules are stored in MongoDB (via fluxgate-mongo-adapter)
 * 2. Rules are loaded using MongoRuleSetProvider
 * 3. Rate limiting is enforced by RedisRateLimiter (via fluxgate-redis-ratelimiter)
 * 4. Token buckets are stored in Redis for distributed enforcement
 * <p>
 * Prerequisites:
 * - MongoDB running on localhost:27017 (or FLUXGATE_MONGO_URI)
 * - Redis running on localhost:6379 (or FLUXGATE_REDIS_URI)
 */
...
class MongoRedisRateLimitIntegrationTest {
    ...
    private static final String MONGO_DB = System.getProperty(
            "fluxgate.mongo.db",
    ...
    @Test
    @Order(1)
    @DisplayName("End-to-End: MongoDB rule storage → Redis rate limiting (100 allowed, 101st rejected)")
    void shouldEnforceRateLimitFromMongoRuleStoredInRedis() {
        ...
        for (int i = 1; i <= 100; i++) {
            ...
            assertTrue(result.isAllowed(),
                    String.format("Request #%d should be allowed", i));

            // Print progress every 20 requests
            if (i % 20 == 0 || i == 1) {
                System.out.printf("  Request #%d: ALLOWED (remaining: %d tokens)%n",
                        i, result.getRemainingTokens());
            }
        }
        ...
        assertTrue(rejectedResult.getNanosToWaitForRefill() > 0,
        ...
        assertNotNull(rejectedResult.getMatchedRule(),
                "Rejected result should have matched rule");
        assertEquals(RULE_ID, rejectedResult.getMatchedRule().getId(),
                "Matched rule ID should be correct");
    ...
    void shouldIsolateRateLimitsByIp() {
        ...
        RateLimitRule rule = RateLimitRule.builder("isolation-test")
    ...
    void shouldCreateCorrectRedisKeysWithTTL() {
        ...
        // Setup rule
        RateLimitBand band = RateLimitBand.builder(Duration.ofSeconds(30), 10)
                .label("10-per-30sec")
                .build();

        RateLimitRule rule = RateLimitRule.builder("redis-key-test")
                .name("Redis Key Test")
                .enabled(true)
                .scope(LimitScope.PER_IP)
                .keyStrategyId("clientIp")
        ...
        for (String key : keys) {
            ...
            assertTrue(value.containsKey("last_refill_nanos"), "Key should have 'last_refill_nanos' field");
        }
