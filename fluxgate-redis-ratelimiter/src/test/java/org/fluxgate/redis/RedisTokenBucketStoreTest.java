...
class RedisTokenBucketStoreTest {
  ...
  void shouldConsumeWhenAvailable() {
    // given
    RateLimitBand band = RateLimitBand.builder(Duration.ofSeconds(1), 10).label("test").build();
  ...
  void shouldRejectWhenNotEnoughTokens() {
    // given
    RateLimitBand band = RateLimitBand.builder(Duration.ofSeconds(1), 5).label("test").build();
