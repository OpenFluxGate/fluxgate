 ...
 *
 * <p>This class handles: - Redis connection setup - Loading production Lua scripts with all
 * critical fixes - TokenBucketStore initialization
 *
 * <p>Production features: - Uses Redis TIME (no clock drift across distributed nodes) - Integer
 * arithmetic only (no precision loss) - Read-only on rejection (fair rate limiting) - TTL safety
 * margin + max cap
 */
