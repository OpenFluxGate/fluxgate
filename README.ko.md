...
FluxGate의 Spring Boot 자동 설정은 기본적으로 fail-closed입니다. 자동 설정을 사용하는 서비스는 버전만 올려도 limiter 장애와 누락된 rule set을 HTTP 429로 거부하며, 신뢰 프록시 뒤에서 `fluxgate.ratelimit.trust-client-ip-header=true`를 명시하지 않는 한 전달된 클라이언트 IP 헤더를 무시합니다.

Redis 버킷 키는 `fluxgate:{ruleSetId}:{ruleId}:{keyValue}:{bandLabel}` 형식을 사용합니다. 공유 Redis 환경에서는 테넌트별 rule set ID를 분리하고 custom key strategy에 테넌트 식별자를 포함하세요. 여러 테넌트가 하나의 global rule set을 공유하는 방식에 의존하면 안 됩니다.
...
| `fluxgate.ratelimit.default-rule-set-id` | -                                      | 기본 규칙 세트 ID                                |
| `fluxgate.ratelimit.failure-behavior`    | `DENY`                                 | limiter 장애 시 `DENY` 또는 `ALLOW`              |
| `fluxgate.ratelimit.missing-rule-behavior` | `DENY`                               | rule set 누락 시 `DENY` 또는 `ALLOW`             |
| `fluxgate.ratelimit.trust-client-ip-header` | `false`                             | 프록시 전달 IP 헤더 신뢰 여부                         |
...
멀티테넌트 시스템에서는 `X-Tenant-Id`를 `RequestContext`에 복사하기 전에 애플리케이션 경계에서 검증해야 합니다. 안전한 custom key는 `tenantId + ":" + userId`처럼 테넌트와 주체를 함께 포함해야 한 테넌트가 다른 테넌트의 버킷을 소비하거나 초기화하지 못합니다.
...
Limiter 장애는 `fluxgate.limiter.failures` 메트릭으로 노출되며 `rule_set`, `endpoint`, `action`, `exception` 태그를 포함합니다. 운영 환경에서는 `action=fail_open`이 0보다 커지면 알림을 걸고, `action=fail_closed`는 의존성 장애 신호로 추적하세요.
...
| `fluxgate_limiter_failures_total` | Counter | action과 exception별 limiter 의존성 장애 수 |
...
# Redis Cluster 통합 테스트 실행 (docker/redis-cluster.yml 또는 localhost:7100-7105 필요)
