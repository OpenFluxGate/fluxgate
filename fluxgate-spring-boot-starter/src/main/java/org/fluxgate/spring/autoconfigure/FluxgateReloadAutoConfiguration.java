...
public class FluxgateReloadAutoConfiguration {
  ...
  public RuleReloadStrategy ruleReloadStrategy(
    ...
    return switch (strategy) {
      case NONE -> {
        ...
        yield new NoOpReloadStrategy();
      }
      case POLLING -> createPollingStrategy(ruleSetProvider, ruleCache);
      case PUBSUB -> {
        if (!redisEnabled) {
          log.warn("PUBSUB strategy requested but Redis is not enabled. Falling back to POLLING.");
          yield createPollingStrategy(ruleSetProvider, ruleCache);
        }
        yield createPubSubStrategy();
      }
      case AUTO -> {
        if (redisEnabled) {
  ...
  public CachingRuleSetProvider cachingRuleSetProvider(
    ...
    if (bucketResetHandler instanceof RedisBucketResetHandler redisBucketResetHandler) {
