...
public class FluxgateRateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        Optional.ofNullable(request.getHeader("User-Agent")).ifPresent(v -> MDC.put(MdcKeys.USER_AGENT, v));
        Optional.ofNullable(request.getHeader("Referer")).ifPresent(v -> MDC.put(MdcKeys.REFERER, v));

        // User identification (if available)
        Optional.ofNullable(request.getHeader(Headers.USER_ID)).ifPresent(v -> MDC.put(MdcKeys.USER_ID, v));
        Optional.ofNullable(request.getHeader(Headers.API_KEY)).ifPresent(v -> MDC.put(MdcKeys.API_KEY, maskSensitive(v)));
