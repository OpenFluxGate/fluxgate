 ...
 * <ul>
 *   <li>{@code fluxgate.requests.total} - Total requests processed (counter)</li>
 *   <li>{@code fluxgate.requests.allowed} - Allowed requests (counter)</li>
 *   <li>{@code fluxgate.requests.rejected} - Rejected/rate-limited requests (counter)</li>
 *   <li>{@code fluxgate.requests.duration} - Request processing duration (timer)</li>
 * </ul>
 * <p>
 * All metrics are tagged with:
 * <ul>
 *   <li>{@code rule_set} - The rule set ID applied</li>
 *   <li>{@code endpoint} - The request endpoint (optional)</li>
 * </ul>
 */
public class FluxgateMetrics {
    ...
    public void recordRequest(String ruleSetId, String endpoint, boolean allowed, Duration duration) {
        ...
        getOrCreateCounter(METRIC_PREFIX + ".requests.total", ruleSetId, endpoint, null)
                .increment();

        // Increment result-specific counter
        getOrCreateCounter(METRIC_PREFIX + ".requests", ruleSetId, endpoint, result)
                .increment();

        // Record duration
        getOrCreateTimer(METRIC_PREFIX + ".requests.duration", ruleSetId, endpoint)
                .record(duration);
    }
    ...
    private Counter getOrCreateCounter(String name, String ruleSetId, String endpoint, String result) {
    ...
    private String buildKey(String name, String ruleSetId, String endpoint, String result) {
        StringBuilder sb = new StringBuilder(name)
                .append(":").append(sanitize(ruleSetId));
        if (endpoint != null) {
