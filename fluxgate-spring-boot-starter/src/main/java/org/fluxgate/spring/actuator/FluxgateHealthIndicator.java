 ...
 * <p>
 * Provides health status for the FluxGate rate limiting system.
 * Reports on the status of:
 * <ul>
 *   <li>Rate limiting enabled/disabled state</li>
 *   <li>MongoDB connection (if enabled)</li>
 *   <li>Redis connection (if enabled)</li>
 * </ul>
 * <p>
 * Access via: {@code GET /actuator/health/fluxgate}
 */
public class FluxgateHealthIndicator implements HealthIndicator {
    ...
    public Health health() {
        ...
        if (properties.getMongo().isEnabled()) {
            try {
                HealthStatus mongoStatus = mongoHealthChecker != null
        ...
        if (properties.getRedis().isEnabled()) {
            try {
                HealthStatus redisStatus = redisHealthChecker != null
