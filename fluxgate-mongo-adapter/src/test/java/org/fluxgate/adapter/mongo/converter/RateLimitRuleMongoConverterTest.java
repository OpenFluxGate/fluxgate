...
class RateLimitRuleMongoConverterTest {
    ...
    void toDto_shouldConvertDomainToDto() {
        // given
        RateLimitRule domainRule = RateLimitRule.builder("test-rule")
                .name("Test Rule")
                .enabled(true)
                .scope(LimitScope.PER_API_KEY)
                .keyStrategyId("apiKey")
                .onLimitExceedPolicy(OnLimitExceedPolicy.REJECT_REQUEST)
                .addBand(RateLimitBand.builder(Duration.ofSeconds(1), 100)
                        .label("per-second")
                        .build())
    ...
    void toDomain_shouldConvertDtoToDomain() {
        ...
        RateLimitRuleDocument dto = new RateLimitRuleDocument(
                "test-rule",
                "Test Rule",
                true,
                LimitScope.PER_IP,
    ...
    void fromBson_shouldConvertBsonToDto() {
        // given
        Document bandBson = new Document()
    ...
    void shouldHandleMultipleBandsInConversion() {
        // given
        RateLimitRule domainRule = RateLimitRule.builder("multi-band-rule")
    ...
    void shouldHandleRoundTripConversion() {
        // given
        RateLimitRule original = RateLimitRule.builder("roundtrip-rule")
    ...
    void shouldHandleDisabledRuleConversion() {
        // given
        RateLimitRule disabledRule = RateLimitRule.builder("disabled-rule")
                .name("Disabled Rule")
    ...
    void shouldHandleAllLimitScopeValues() {
        LimitScope[] scopes = {LimitScope.GLOBAL, LimitScope.PER_IP, LimitScope.PER_USER, LimitScope.PER_API_KEY};

        for (LimitScope scope : scopes) {
            // given
            RateLimitRule rule = RateLimitRule.builder("scope-test-" + scope)
                    .name("Scope Test")
                    .enabled(true)
                    .scope(scope)
                    .keyStrategyId("test")
