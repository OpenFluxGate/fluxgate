 ...
 *
 * <p>Implementations can use various storage backends: - MongoDB (fluxgate-mongo-adapter) -
 * JDBC/JPA (custom implementation) - In-memory (for testing) - File-based (YAML/JSON)
 *
 * <p>This interface uses core domain objects ({@link RateLimitRule}), not storage-specific
 * documents.
 */
public interface RateLimitRuleRepository {
  ...
  /**
   * Save or update a rule. If a rule with the same ID exists, it will be replaced.
   *
