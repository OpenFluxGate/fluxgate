 ...
 * <p>
 * OnLimitExceedPolicy defines how the engine should behave when a limit is exceeded:
 * - REJECT_REQUEST: Immediately reject the request
 * - WAIT_FOR_REFILL: Block the caller until enough tokens are available
 */
...
class OnLimitExceedPolicyTest {
    ...
    void valueOf_shouldReturnCorrectEnumConstant() {
        ...
        assertEquals(OnLimitExceedPolicy.WAIT_FOR_REFILL, OnLimitExceedPolicy.valueOf("WAIT_FOR_REFILL"));
    }
    ...
    void valueOf_shouldThrowForInvalidName() {
        ...
        assertThrows(IllegalArgumentException.class, () -> OnLimitExceedPolicy.valueOf("reject_request")); // case-sensitive
    }
