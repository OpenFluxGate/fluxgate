 ...
 * <p>
 * LimitScope defines the logical scope of a rate limit:
 * - GLOBAL: Single global bucket for all traffic
 * - PER_API_KEY: One bucket per API key
 * - PER_USER: One bucket per user identifier
 * - PER_IP: One bucket per client IP address
 * - CUSTOM: Custom scope resolved by a pluggable key strategy
 */
...
class LimitScopeTest {
    ...
    void valueOf_shouldThrowForInvalidName() {
        ...
        assertThrows(IllegalArgumentException.class, () -> LimitScope.valueOf("global")); // case-sensitive
    }
