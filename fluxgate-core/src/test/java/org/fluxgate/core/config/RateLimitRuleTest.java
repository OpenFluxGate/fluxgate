 ...
 *
 * <p>RateLimitRule is the core configuration object describing a rate limit rule. It is
 * intentionally storage-agnostic and engine-agnostic.
 */
...
class RateLimitRuleTest {
  ...
  class BasicHappyPathTests {
    ...
    void build_shouldPreserveBandOrder() {
      ...
      // when
      RateLimitRule rule =
          RateLimitRule.builder("rule-1").addBand(band1).addBand(band2).addBand(band3).build();
  ...
  class DefaultBehaviorTests {
    ...
    void build_shouldUseIdAsDefaultNameWhenNameNotProvided() {
      ...
      // when
      RateLimitRule rule =
          RateLimitRule.builder(id).addBand(createBand(Duration.ofMinutes(1), 100)).build();
  ...
  class BandsRequirementTests {
    ...
    void build_shouldThrowWhenNoBandsConfigured() {
      ...
      IllegalArgumentException exception =
          assertThrows(
              IllegalArgumentException.class, () -> RateLimitRule.builder("rule-1").build());
  ...
  class NullSafetyTests {
    ...
    void builder_shouldThrowWhenIdIsNull() {
      ...
      NullPointerException exception =
          assertThrows(NullPointerException.class, () -> RateLimitRule.builder(null));
    ...
    void addBand_shouldThrowWhenNull() {
      ...
      NullPointerException exception =
          assertThrows(
              NullPointerException.class, () -> RateLimitRule.builder("rule-1").addBand(null));
  ...
  class EdgeCaseTests {
    ...
    void build_shouldAllowAllLimitScopeValues() {
      ...
      for (LimitScope scope : LimitScope.values()) {
        RateLimitRule rule =
            RateLimitRule.builder("rule-" + scope.name())
                .scope(scope)
                .addBand(createBand(Duration.ofMinutes(1), 100))
                .build();
        assertEquals(scope, rule.getScope());
    ...
    void build_shouldAllowAllOnLimitExceedPolicyValues() {
      ...
      for (OnLimitExceedPolicy policy : OnLimitExceedPolicy.values()) {
        RateLimitRule rule =
            RateLimitRule.builder("rule-" + policy.name())
                .onLimitExceedPolicy(policy)
                .addBand(createBand(Duration.ofMinutes(1), 100))
                .build();
        assertEquals(policy, rule.getOnLimitExceedPolicy());
