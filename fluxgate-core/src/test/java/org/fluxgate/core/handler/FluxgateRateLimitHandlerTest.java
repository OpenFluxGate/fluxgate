 ...
 *
 * <p>FluxgateRateLimitHandler is the core interface for rate limit handling. Implementations decide
 * how to perform rate limiting: - API-based: Call external FluxGate API server - Redis direct: Use
 * Redis rate limiter directly - Standalone: In-memory rate limiting (for testing)
 */
...
class FluxgateRateLimitHandlerTest {
  ...
  class InterfaceContractTests {
    ...
    void shouldSupportCustomImplementation() {
      ...
      final int[] counter = {0};
      FluxgateRateLimitHandler countingHandler =
          (context, ruleSetId) -> {
            counter[0]++;
            return counter[0] <= 3
    ...
    void shouldSupportContextAwareImplementation() {
      ...
      FluxgateRateLimitHandler contextAwareHandler =
          (context, ruleSetId) -> {
            // Allow VIP users
      ...
      RequestContext vipContext =
          RequestContext.builder().userId("vip-user").attribute("tier", "vip").build();

      RequestContext regularContext =
          RequestContext.builder().userId("regular-user").attribute("tier", "regular").build();
  ...
  class EdgeCaseTests {
    ...
    void customHandler_canThrowExceptions() {
      // given
      FluxgateRateLimitHandler failingHandler =
          (context, ruleSetId) -> {
