 ...
 *
 * <p>RateLimitResult represents the result of a single rate limit evaluation.
 */
...
class RateLimitResultTest {
  ...
  class BuilderTests {
    ...
    void builder_shouldThrowWhenRejectedWithNullKey() {
      ...
      IllegalStateException exception =
          assertThrows(
              ...
              () -> RateLimitResult.builder(null).allowed(false).build());
    ...
    void builder_shouldAllowNullKeyForAllowedResult() {
      // given / when
      RateLimitResult result =
          RateLimitResult.builder(null).allowed(true).remainingTokens(100).build();
  ...
  class GetterTests {
    ...
    void isAllowed_shouldReturnCorrectValue() {
      ...
      // when / then - allowed
      RateLimitResult allowedResult = RateLimitResult.builder(key).allowed(true).build();
      assertTrue(allowedResult.isAllowed());

      // when / then - rejected
      RateLimitResult rejectedResult = RateLimitResult.builder(key).allowed(false).build();
  ...
  class EdgeCaseTests {
    ...
    void shouldHandleLargeNanosToWaitValues() {
      ...
      // when
      RateLimitResult result =
          RateLimitResult.rejected(createKey("test"), createRule(), largeNanos);
    ...
    void shouldHandleZeroNanosToWaitForRejected() {
      ...
      // given / when
      RateLimitResult result = RateLimitResult.rejected(createKey("test"), createRule(), 0L);
